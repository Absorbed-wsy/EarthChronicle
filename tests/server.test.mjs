...
test('content server is optional, web is read-only even on localhost, port collisions preserve the running listener',async()=>{
 ...
 try{
  ...
  const ordinarySession=await fetch(local.base+'/api/session').then(r=>r.json());assert.equal(ordinarySession.interface,'web');assert.equal(ordinarySession.canEdit,false);assert.equal(ordinarySession.token,null);assert.equal(ordinarySession.version,'0.1.11');
  ...
  for(const file of ['/','/app.js','/style.css','/maps/liberty.json']){const response=await fetch(local.base+file);assert.equal(response.status,200);assert.equal(response.headers.get('cache-control'),'no-store');assert.equal(response.headers.get('x-earth-chronicle-version'),'0.1.11');await response.arrayBuffer();}
  const health=await fetch(local.base+'/api/health').then(r=>r.json());assert.deepEqual(health,{appId:'earth-chronicle',version:'0.1.11'});
