...
test('bundled historical content has valid linked data',async()=>{
  assert.ok(bundled.events.filter(e=>e.id.startsWith('ming-')).length>=24,'retain the original Ming collection');assert.equal(new Set(bundled.events.map(e=>e.id)).size,bundled.events.length);for(const event of bundled.events){assert.ok(bundled.places.some(p=>p.id===event.placeId));assert.ok(safeSourceUrl(event.sourceUrl));assert.ok(['year','month','day'].includes(event.precision));}
});
...
test('content server is optional, web is read-only even on localhost, port collisions preserve the running listener',async()=>{
 ...
 try{
  ...
  const ordinarySession=await fetch(local.base+'/api/session').then(r=>r.json());assert.equal(ordinarySession.interface,'web');assert.equal(ordinarySession.canEdit,false);assert.equal(ordinarySession.token,null);assert.equal(ordinarySession.version,'0.1.25');
  ...
  for(const file of ['/','/app.js','/style.css','/maps/liberty.json']){const response=await fetch(local.base+file);assert.equal(response.status,200);assert.equal(response.headers.get('cache-control'),'no-store');assert.equal(response.headers.get('x-earth-chronicle-version'),'0.1.25');await response.arrayBuffer();}
  const health=await fetch(local.base+'/api/health').then(r=>r.json());assert.deepEqual(health,{appId:'earth-chronicle',version:'0.1.25'});
