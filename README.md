...
当前源码内置 3117 条事件、692 个地点，时间范围从公元前 770 年至 2026 年，覆盖春秋战国、秦汉、三国两晋南北朝、隋唐五代、宋辽金夏元、明清及近现代。古代部分提供主要历史脉络，清代含 361 条、中华民国含 306 条、中华人民共和国含 775 条；各条记录包含年份、摘要、来源与地点说明。古代资料通常仅精确到年，约年与纪年差异在详情说明；同期政权按事件归属区分，避免重复。县域与场址记录支持县名、旧称和所属地级市检索。这是重大事件选编，尚非完整通史；最新收录节点为 2026 年 3 月 12 日。
...
在 [v0.1.25 Release](https://github.com/Absorbed-wsy/EarthChronicle/releases/tag/v0.1.25) 下载 `EarthChronicle-v0.1.25-windows-x64.zip`，解压到可写目录，双击其中的 **EarthChronicle.exe**。发行包已包含 Node.js、地图、字形及程序组件，无需构建或安装 Node.js；请保留完整文件夹。
...
打包脚本重新编译 EXE，校验必要资源并按 `package.json` 版本号生成便携 ZIP（当前为 `dist/EarthChronicle-v0.1.25-windows-x64.zip`）和校验文件。包中不包含个人数据库、日志、浏览器缓存或开发工具。已完成构建时可用 `-SkipBuild`。
...
| 资源 | 随源码提供的内容 |
| --- | --- |
| `public/data/history.json` | 春秋战国至现代的事件、地点、已核实日期及来源 |
...
首次启动会自动创建 `data/chronicle.sqlite` 并导入基础历史资料。可选择“近现代”合并浏览 1840 年以来的资料，或选择“春秋战国”“秦”“西汉”“唐”“宋”“明”“清”等限定时期，再按年份、城市和类型浏览；选择“全部年份”可阅读所选时期的列表，地图仍只显示当前年份事件。当前年份没有资料时，列表和地图事件标记为空。资料聚焦中国历史，包含部分发生于境外的中国相关事件；国家列表不代表各国通史均已收录。
