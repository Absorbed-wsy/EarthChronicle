...
import {openChronicleDatabase,RequestError} from './database.mjs';
const ROOT=path.dirname(fileURLToPath(import.meta.url)),VERSION='0.3.3';
...
async function closeListener(server){if(!server?.listening)return;await new Promise(resolve=>{server.close(resolve);server.closeIdleConnections();});}
...
export async function createChronicleServer(options={}){
 ...
 async function changeContent(config){
  ...
  try{
   if(config.enabled&&(!webServer?.listening||config.port!==contentPort)){
    ...
    if(closing){await closeListener(candidate);throw new RequestError(503,'软件正在关闭，不能再开启内容服务器。');}
    const previous=webServer;webServer=candidate;contentPort=config.port;await closeListener(previous);
   }else if(!config.enabled){const previous=webServer;webServer=null;await closeListener(previous);contentPort=config.port;}
 ...
 function createListener(isWeb){
  const listener=http.createServer(async(req,res)=>{
   res.setHeader('X-Content-Type-Options','nosniff');res.setHeader('Referrer-Policy','no-referrer');res.setHeader('Content-Security-Policy',"frame-ancestors 'self'");res.setHeader('X-Earth-Chronicle-Version',VERSION);
   try{
    ...
    if(route==='/api/geology/manifest'&&req.method==='GET'){authorizeRead(req,isWeb);return json(res,200,database.geologyManifest());}
    if(/^\/api\/geology\/\d+$/.test(route)&&req.method==='GET'){authorizeRead(req,isWeb);const snapshot=database.geologySnapshot(Number(route.split('/').at(-1)));if(!snapshot)throw new RequestError(404,'未收录这个地质切片。');return json(res,200,snapshot);}
    ...
    if(route==='/api/database/export'&&req.method==='GET'){authorize(req,isWeb);const bytes=await database.exportDatabase();res.writeHead(200,{'Content-Type':'application/vnd.sqlite3','Content-Disposition':'attachment; filename="earthchronicle.sqlite"','Cache-Control':'no-store','Content-Length':bytes.length});return res.end(bytes);}
    if(route==='/api/database/import'&&req.method==='POST'){authorize(req,isWeb);if(!String(req.headers['content-type']||'').startsWith('application/octet-stream'))throw new RequestError(415,'请上传数据库文件。');return json(res,200,await database.importDatabase(await readBody(req,128*1024*1024)));}
    ...
    const extension=path.extname(target).toLowerCase();res.writeHead(200,{'Content-Type':MIME[extension]||'application/octet-stream','Content-Length':fileStat.size,'Cache-Control':['.html','.js','.mjs','.json','.css'].includes(extension)?'no-store':'public, max-age=86400'});if(req.method==='HEAD')return res.end();const stream=createReadStream(target);stream.on('error',()=>res.destroy());stream.pipe(res);
   }catch(error){if(res.headersSent)return res.destroy();if(!(error instanceof RequestError))console.error('[EarthChronicle]',error.message);json(res,error instanceof RequestError?error.status:500,{error:error instanceof RequestError?error.message:'读取或保存时发生错误，请检查磁盘空间及文件权限。'});}
  });listener.requestTimeout=60000;listener.headersTimeout=10000;return listener;
 }
 const server=createListener(false);
 async function close(){if(closing)return closing;closing=(async()=>{await Promise.all([closeListener(server),closeListener(webServer)]);database.close();})();return closing;}
